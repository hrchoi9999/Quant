...
# 2026-02-24 세션 종료 반영(최종 상태 업데이트)

> 원칙: 기존 본문은 유지하고, 변경점은 아래에 “추가/정정”으로 누적 기록합니다.

## 1) 이번 세션에서 실제로 해결된 것(확정)

### (1) Google Sheet 업로드: Refactor 엔진 단독 성공
- Refactor 실행에서 gsheet 업로드가 **정상 동작**함을 확인.
- 주요 이슈 및 조치 요약:
  - gsheet 플러그인 호출 시그니처 불일치 → **호환 레이어 적용**
  - `DatetimeIndex` truth-value 오류(week_anchor_dates/next_trading_day) → **len 기반 체크로 수정**
  - 시트 탭(range) 파싱 오류(`Unable to parse range ...`) → **탭 자동 생성 + 안전 quoting**
  - credential 경로 불일치(`.\secrets\...` vs 실제 `D:\Quant\config\...`) → **config 경로 사용**

### (2) Windows(perf_windows) 값이 0으로 나오던 문제 해결(핵심)
- 증상: `equity`가 기간 내내 1.0 고정, `port_ret`가 0.0 고정 → perf_windows가 전부 0.0
- 원인: `next_trading_day()`가 “다음 거래일”이 아니라 **현재일을 반환**하여,
  pending 주문 체결 루프가 실행되지 않는 구조적 버그.
- 조치: `core/data.py::next_trading_day()`를 **실제 다음 거래일(T+1) 반환**으로 수정.
- 결과:
  - `regime_bt_equity_*.csv`에서 equity/port_ret 변동 확인
  - `regime_bt_perf_windows_*.csv`에서 1/2/3/5Y 지표가 0이 아닌 값으로 채워짐
  - 구글시트 `*_windows` 탭에 값 업로드 확인

## 2) 현재 Canonical 기준(정정)

- ~~레거시 parity(완전 동치)가 목표~~  
  → **정정: Refactor 결과가 Canonical, 레거시는 참고용**

단, 레거시에서 확정된 “거래/날짜 규약” 중 프로젝트가 채택한 항목은 계속 유지합니다:
- Decision date(리밸런스 결정일) vs Execution date(T+1 체결일) 분리
- fee/slippage 표준: 5/5 = 총 10bps

## 3) 잔여 리스크/주의(남아있는 가능성)

- `gsheet_uploader` 유틸 함수명/인터페이스가 환경별로 다를 수 있으므로,
  gsheet_plugin은 **직접 Google API 업로드 fallback**을 유지합니다.
- 향후 리팩토링 시 `core/data.py`의 날짜 관련 유틸은
  pandas Index 타입 혼선을 다시 만들기 쉬우므로,
  `len(dates)==0` / `dates.empty` 등 타입 안전 규약을 유지해야 합니다.

## 4) 다음 세션 시작 시 확인해야 할 “정상 동작 체크” (스모크)

1) 실행(대표 커맨드) 후 8종 CSV 생성 확인
2) `regime_bt_equity_*.csv`에서 equity/port_ret가 0 고정이 아닌지 확인
3) `regime_bt_perf_windows_*.csv`에서 1/2/3/5Y의 지표가 0이 아닌지 확인
4) gsheet 탭 `{
   <prefix>_snapshot, <prefix>_windows, <prefix>_trades, <prefix>_ledger, <prefix>_summary
   }` 업로드 확인
