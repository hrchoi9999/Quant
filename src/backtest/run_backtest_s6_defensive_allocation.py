...
def _ensure_task04_baseline(asof: str, start: str, end: str, rebalance: str, outdir: Path) -> Path:
    baseline = outdir / f'etf_alloc_summary_{asof.replace("-", "")}_{rebalance}_{start.replace("-", "")}_{end.replace("-", "")}_risk_off.csv'
    if baseline.exists():
        return baseline
    cmd = [
        str(PROJECT_ROOT / 'venv64' / 'Scripts' / 'python.exe'),
        str(PROJECT_ROOT / 'src' / 'backtest' / 'run_backtest_etf_allocation.py'),
        '--asof', asof, '--start', start, '--end', end, '--rebalance', rebalance, '--force-mode', 'risk_off', '--outdir', str(outdir)
    ]
...
def main() -> None:
    ap = argparse.ArgumentParser(description='Run S6 defensive allocation backtest.')
    ...
    cfg = S6DefensiveConfig(execution=S6ExecutionConfig(
    ...
    if close_wide.empty:
        raise RuntimeError('No ETF prices loaded for S6 run.')
    ...
    result = run_s6_backtest(close_wide=close_wide, ret_wide=ret_wide, core_df=core_df, rebalance_dates=rebalance_dates, cfg=cfg, name_map=name_map, market_map=market_map)

    stamp = f"{asof.replace('-', '')}_{str(args.rebalance).upper()}_{start.replace('-', '')}_{end.replace('-', '')}"
    summary_path = outdir / f's6_alloc_summary_{stamp}.csv'
    equity_path = outdir / f's6_alloc_equity_{stamp}.csv'
    weights_path = outdir / f's6_alloc_weights_{stamp}.csv'
    trades_path = outdir / f's6_alloc_trades_{stamp}.csv'
    compare_path = outdir / f's6_vs_task04_riskoff_{stamp}.csv'
    ...
    s6 = pd.read_csv(summary_path)
    compare = pd.DataFrame([
        {'model': 'TASK04_RISK_OFF', **base.iloc[0].to_dict()},
        {'model': 'S6_DEFENSIVE_V1', **s6.iloc[0].to_dict()},
    ])
