    ...
    *,
    ...
    for dt in dates:
        ...
        if pd.Timestamp(dt) in rb_set:
            ...
            alloc = allocate_s5_neutral(core_df=core_df, close_wide=close_wide, asof=pd.Timestamp(dt), cfg=cfg, available_tickers=available_tickers)
            ...
            for row in alloc.selection_df.to_dict('records'):
                ...
                weights_rows.append({
                    ...
                    'mode': 'neutral', 'model': cfg.strategy_name, 'ticker': ticker, 'name': row.get('name', 'CASH' if ticker == 'CASH' else ''),
        ...
        equity_rows.append({
            'date': pd.Timestamp(dt).strftime('%Y-%m-%d'), 'port_ret': float(eq / eq_prev - 1.0), 'equity': float(eq), 'mode': 'neutral',
