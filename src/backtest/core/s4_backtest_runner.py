...
def run_s4_backtest(
    *,
    ...
    value_wide = value_wide.copy(); value_wide.columns = price_cols
    ...
    for dt in dates:
        ...
        if pending is not None and pd.Timestamp(dt) == pending.exec_date:
            ...
            for ticker in sorted(set(prev_w.index).union(set(w.index))):
                ...
                if abs(delta) < 1e-12:
                    continue
                trades_rows.append({
                    'rebalance_date': pending.decision_date.strftime('%Y-%m-%d'), 'trade_date': pending.exec_date.strftime('%Y-%m-%d'),
                    'side': 'BUY' if delta > 0 else 'SELL', 'ticker': ticker, 'name': name_map.get(ticker, ticker), 'market': market_map.get(ticker, 'ETF'),
                    'prev_weight': prev_weight, 'new_weight': new_weight, 'delta_weight': delta,
                    'exec_price': float(close_wide.loc[dt, ticker]) if ticker in close_wide.columns else float('nan'),
                    'turnover_component': abs(delta) / 2.0, **last_diag,
                })
            pending = None
        ...
        if pd.Timestamp(dt) in rb_set:
            ...
            alloc = allocate_s4_risk_on(core_df=core_df, close_wide=close_wide, value_wide=value_wide, asof=pd.Timestamp(dt), cfg=cfg, available_tickers=available_tickers)
            ...
            pending = _PendingTrade(pd.Timestamp(dt), pd.Timestamp(trade_dt), target, total_cost, turnover, dict(alloc.diagnostics))
            for row in alloc.selection_df.to_dict('records'):
                ticker = str(row.get('ticker', ''))
                weights_rows.append({
                    'rebalance_date': pd.Timestamp(dt).strftime('%Y-%m-%d'), 'trade_date': pd.Timestamp(trade_dt).strftime('%Y-%m-%d'),
                    'mode': 'risk_on', 'model': cfg.strategy_name, 'ticker': ticker, 'name': row.get('name', 'CASH' if ticker == 'CASH' else ''),
                    'market': market_map.get(ticker, 'CASH' if ticker == 'CASH' else 'ETF'), 'group_key': row.get('group_key', ''),
                    'target_group_weight': float(row.get('target_group_weight', 0.0) or 0.0), 'weight': float(row.get('assigned_weight', 0.0) or 0.0),
                    'selected': bool(row.get('selected', False)), 'available': bool(row.get('available', False)), **alloc.diagnostics,
                })

        gross = float(w.sum())
        cash_weight = max(0.0, 1.0 - gross)
        equity_rows.append({
            'date': pd.Timestamp(dt).strftime('%Y-%m-%d'), 'port_ret': float(eq / eq_prev - 1.0), 'equity': float(eq), 'mode': 'risk_on',
            'model': cfg.strategy_name, 'n_holdings': int((w > 0).sum()), 'gross_exposure': gross, 'cash_weight': cash_weight, **last_diag,
        })
    ...
    summary_df = pd.DataFrame([{
        'strategy': cfg.strategy_name, 'start': str(equity_df['date'].iloc[0]), 'end': str(equity_df['date'].iloc[-1]), 'days': int(len(equity_df)),
        'cagr': perf['cagr'], 'mdd': perf['mdd'], 'sharpe': perf['sharpe'], 'avg_daily_ret': perf['avg_daily_ret'], 'vol_daily': perf['vol_daily'],
        'turnover': float(turnover_total), 'rebalance_count': int(len(rebalance_dates)), 'fee_bps': float(cfg.execution.fee_bps), 'slippage_bps': float(cfg.execution.slippage_bps),
        'cagr_1y': w1['cagr'], 'sharpe_1y': w1['sharpe'], 'mdd_1y': w1['mdd'], 'cagr_2y': w2['cagr'], 'sharpe_2y': w2['sharpe'], 'mdd_2y': w2['mdd'],
        'cagr_3y': w3['cagr'], 'sharpe_3y': w3['sharpe'], 'mdd_3y': w3['mdd'], 'cagr_5y': w5['cagr'], 'sharpe_5y': w5['sharpe'], 'mdd_5y': w5['mdd'],
    }])
