...
try:
    from src.backtest.configs.s5_neutral_config import S5NeutralConfig, S5ExecutionConfig
...
def _ensure_task04_baseline(asof: str, start: str, end: str, rebalance: str, outdir: Path) -> Path:
    baseline = outdir / f'etf_alloc_summary_{asof.replace("-", "")}_{rebalance}_{start.replace("-", "")}_{end.replace("-", "")}.csv'
    if baseline.exists():
        return baseline
    cmd = [
        str(PROJECT_ROOT / 'venv64' / 'Scripts' / 'python.exe'),
        str(PROJECT_ROOT / 'src' / 'backtest' / 'run_backtest_etf_allocation.py'),
        '--asof', asof, '--start', start, '--end', end, '--rebalance', rebalance, '--outdir', str(outdir)
    ]
...
def main() -> None:
    ap = argparse.ArgumentParser(description='Run S5 neutral mean-reversion allocation backtest.')
    ...
    cfg = S5NeutralConfig(execution=S5ExecutionConfig(
    ...
    if close_wide.empty:
        raise RuntimeError('No ETF prices loaded for S5 run.')
    ...
    result = run_s5_backtest(close_wide=close_wide, ret_wide=ret_wide, core_df=core_df, rebalance_dates=rebalance_dates, cfg=cfg, name_map=name_map, market_map=market_map)

    stamp = f"{asof.replace('-', '')}_{str(args.rebalance).upper()}_{start.replace('-', '')}_{end.replace('-', '')}"
    summary_path = outdir / f's5_alloc_summary_{stamp}.csv'
    equity_path = outdir / f's5_alloc_equity_{stamp}.csv'
    weights_path = outdir / f's5_alloc_weights_{stamp}.csv'
    trades_path = outdir / f's5_alloc_trades_{stamp}.csv'
    compare_path = outdir / f's5_vs_task04_neutral_{stamp}.csv'
    ...
    s5 = pd.read_csv(summary_path)
    compare = pd.DataFrame([
        {'model': 'TASK04_NEUTRAL', **base.iloc[0].to_dict()},
        {'model': 'S5_NEUTRAL_V1', **s5.iloc[0].to_dict()},
    ])
