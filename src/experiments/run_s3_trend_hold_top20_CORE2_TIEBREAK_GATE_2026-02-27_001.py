# run_s3_trend_hold_top20_CORE2_TIEBREAK_GATE_2026-02-27_001.py ver 2026-02-27_001
import sqlite3
import csv
...
class Params:
    ...
    # =========================
    # Market Gate (리스크 관리 B)
    # - "시장/브레드스(th) 상태가 나쁘면 신규 진입을 막고(또는 포지션을 축소) 현금 비중을 키움"
    # - index 데이터를 강제하지 않기 위해, universe 내부의 "상승 추세 종목 비율(브레드스)"로 게이트를 구성
    # =========================
    market_gate_enabled: bool = True
    market_gate_breadth_min: float = 0.55   # (0~1) universe 중 ma60>ma120 & ma60_slope>0 비율이 이 값 이상이면 gate OPEN
    market_gate_use_ma_stack: bool = True
    market_gate_use_ma60_slope_pos: bool = True
...
def _calc_market_breadth(df_scored: pd.DataFrame, params: Params) -> float:
    """Universe 내부의 '상승 추세 종목 비율'을 계산합니다.
    - ma60/ma120/ma60_slope가 유효한 종목만 대상으로 계산
    - 기본 정의: (ma60 > ma120) AND (ma60_slope > 0)
    """
    d = df_scored.copy()
    ...
    ok = d[need_cols].notna().all(axis=1)
    if ok.sum() == 0:
        return float("nan")
    ...
    if params.market_gate_use_ma_stack:
        cond &= (d["ma60"] > d["ma120"])
    if params.market_gate_use_ma60_slope_pos:
        cond &= (d["ma60_slope"] > 0)

    breadth = float((cond & ok).sum() / ok.sum())
    return breadth


def _is_gate_open(df_scored: pd.DataFrame, params: Params) -> Tuple[bool, float]:
    """게이트(open/close) 판정 + breadth 반환"""
    if not params.market_gate_enabled:
        return True, float("nan")
    breadth = _calc_market_breadth(df_scored, params)
    if not np.isfinite(breadth):
        # 데이터가 없으면 보수적으로 CLOSE
        return False, breadth
    return (breadth >= params.market_gate_breadth_min), breadth
...
def run(params: Params) -> None:
    ...
    # 보유 상태
    holdings: Dict[str, Dict] = {}  # ticker -> {"below_ma60_streak": int, "slope_nonpos_streak": int, "entry_date": str}
    nav = 1.0
    nav_rows = []
    holdings_rows = []
    ...
    for i in range(1, len(rebals)):
        ...
        # --- (3.5) MARKET GATE: universe breadth로 신규진입 허용/차단
        gate_open, gate_breadth = _is_gate_open(df_scored, params)
        ...
        # --- (5) ENTRY: 부족분만큼 신규 편입 (게이트가 OPEN일 때만)
        ...
        elif need > 0 and (not gate_open):
            # 게이트 CLOSED: 신규 진입은 막고, 기존 보유는 exit 규칙대로만 관리.
            # 결과적으로 현금 비중이 증가할 수 있습니다.
            pass
        ...
        nav_rows.append({"date": d, "nav": nav, "holdings": len(held), "cash_weight": cash_weight, "gate_open": int(gate_open), "gate_breadth": gate_breadth})
    ...
    out_nav = OUTDIR / f"s3_nav_hold_top{params.top_n}_core2_gate_{params.start}_{params.end}.csv"
    out_hist = OUTDIR / f"s3_holdings_history_top{params.top_n}_core2_gate_{params.start}_{params.end}.csv"
    out_last = OUTDIR / f"s3_holdings_last_top{params.top_n}_core2_gate_{params.end}.csv"
    ...
    try:
        ...
        out_last_alt = OUTDIR / f"s3_holdings_last_top{params.top_n}_core2_gate_{params.end}_{ts}.csv"
