...
class InstrumentRepository:
    ...
    def _ensure_etf_meta_role_columns(self) -> None:
        with self.connect() as con:
            existing = {row[1] for row in con.execute("PRAGMA table_info(etf_meta)").fetchall()}
            migrations = {
    ...
    def get_etf_core_universe(self, asof: str | None = None, group_key: str | None = None) -> pd.DataFrame:
        self._ensure_etf_meta_role_columns()
        ...
        SELECT im.ticker, im.name, im.asset_type, im.market, im.is_active, im.asof,
               ...
               em.role_key, em.role_confidence, em.role_reason, em.role_schema_version, em.purity_issue
        FROM etf_meta em
