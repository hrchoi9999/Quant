# English filename: 04_factor_evaluation_py.py
# Original path: [능률협회]머신러닝 기반 증권모델링 및 트레이딩 과정 (수강생)/1일차 머신러닝 분석을 위한 알파 팩터 수집 및 작성/2_ alpha_factor_library/04_factor_evaluation_clear(오래걸림).py
# Original filename: 04_factor_evaluation_clear(오래걸림).py
...
# ---- Cell ----
mask = np.triu(np.ones_like(corr_formula, dtype=bool))
fig, ax = plt.subplots(figsize=(22, 18))
cmap = sns.diverging_palette(10, 220, as_cmap=True)
