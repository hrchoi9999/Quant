...
def _pick_group_ticker(core_df: pd.DataFrame, group_key: str, available: set[str] | None) -> tuple[str, str, float] | None:
...
def _clip_and_normalize(weights: dict[str, float], cfg: S6DefensiveConfig) -> dict[str, float]:
    from src.quant2.operations.common_20260922 import bounded_normalize
    maximum = dict(cfg.bounds.max_weights)
    maximum['hedge_inverse_kr'] = min(maximum.get('hedge_inverse_kr', 1.), cfg.bounds.inverse_cap)
    return bounded_normalize(weights, cfg.bounds.min_weights, maximum)
...
def allocate_s6_defensive(
    *,
    ...
    available = None if available_tickers is None else {str(t).zfill(6) for t in available_tickers}
    picks = {}
