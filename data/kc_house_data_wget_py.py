# English filename: kc_house_data_wget_py.py
# Original path: [능률협회]머신러닝 기반 증권모델링 및 트레이딩 과정 (수강생)/7일차 선형모델을 통한 수익률 예측 I/보조실습_앙상블_수업/kc_house_data_wget.py
# Original filename: kc_house_data_wget.py
...
lgb_model = lgb.train(params = lgb_param, train_set = lgb_dtrain) # 학습 진행. .train은 .fit에 해당한다.
lgb_model_predict = lgb_model.predict(test_x) # test data 예측
print("RMSE: {}".format(sqrt(mse(lgb_model_predict, test_y)))) # RMSE
...
shap.force_plot(explainer.expected_value, shap_values[20,:], test_x.iloc[20,:])
