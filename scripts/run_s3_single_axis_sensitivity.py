...
OUTDIR = PROJECT_ROOT / r"reports\model_upgrade_research\20260331\S3_SINGLE_AXIS_SENSITIVITY"
TOP_N = 20
...
AXIS_GRIDS = {
    "momentum_block": [0.15, 0.25, 0.35, 0.40, 0.45],
    "fund_accel": [0.15, 0.20, 0.25, 0.35],
    "trend_block": [0.05, 0.10, 0.20, 0.30],
}
...
def latest_s3_dates() -> list[pd.Timestamp]:
    ...
    dates = read_sql(
        QS_DETAIL_DB,
        "SELECT DISTINCT date FROM run_signal_details_s3 WHERE run_id=? ORDER BY date",
        (run_id,),
        parse_dates=["date"],
    )
...
def build_bucket_panel() -> pd.DataFrame:
    base = read_sql(
        RESEARCH_DB,
        """
        SELECT model_code, horizon, signal_date, end_date, ticker, selected, fwd_ret, path_mdd, top_50pct_flag
...
def build_weights(axis: str, target_value: float) -> dict[str, float]:
    others = {k: v for k, v in BASE_BLOCKS.items() if k != axis}
    remaining = 1.0 - target_value
    base_other_sum = sum(others.values())
    weights = {axis: target_value}
    for k, v in others.items():
        weights[k] = remaining * (v / base_other_sum)
    return weights
...
def build_variant_specs() -> list[dict[str, object]]:
    ...
    for axis, values in AXIS_GRIDS.items():
        for value in values:
            blocks = build_weights(axis, float(value))
            specs.append(
                {
                    "axis": axis,
                    "target_value": float(value),
                    "variant_id": f"{axis}_{int(round(value * 100)):02d}",
                    "fund_level_w": blocks["fund_level"],
                    "fund_accel_w": blocks["fund_accel"],
                    "momentum_block_w": blocks["momentum_block"],
                    "trend_block_w": blocks["trend_block"],
                    "mom20_w": blocks["momentum_block"] * MOMENTUM_SPLIT["mom20"],
                    "vol_ratio_w": blocks["momentum_block"] * MOMENTUM_SPLIT["vol_ratio"],
                    "breakout_w": blocks["momentum_block"] * MOMENTUM_SPLIT["breakout"],
                }
            )
    return specs


def build_s3_score(universe: pd.DataFrame, p_row: pd.DataFrame, fund_snap: pd.DataFrame, spec: dict[str, object]) -> pd.DataFrame:
    ...
    snap["s3_score"] = (
        float(spec["fund_level_w"]) * snap["fund_level_pct"].fillna(0)
        + float(spec["fund_accel_w"]) * snap["fund_accel_pct"].fillna(0)
        + float(spec["mom20_w"]) * snap["mom20_pct"].fillna(0)
        + float(spec["vol_ratio_w"]) * snap["vol_ratio_pct"].fillna(0)
        + float(spec["breakout_w"]) * snap["breakout60"].fillna(0).astype(float)
        + float(spec["trend_block_w"]) * trend_bonus
    )
...
def summarize_accuracy(selected_panel: pd.DataFrame, bucket_panel: pd.DataFrame) -> pd.DataFrame:
    rows = []
    variant_ids = sorted(selected_panel["variant_id"].dropna().unique())
...
def render_md(summary: pd.DataFrame) -> str:
    lines = ["# S3 Single-Axis Sensitivity Analysis", ""]
    lines.append("Single-axis sensitivity test for S3 using normalized block weights. One axis changes at a time, and the remaining blocks are scaled proportionally to keep total score weight = 1.0.")
    lines.append("")
    for axis, ag in summary.groupby("axis"):
        ...
        lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
        for r in ag.sort_values("target_value").itertuples(index=False):
            lines.append(
                f"| {r.variant_id} | {r.fund_level_w:.2f} | {r.fund_accel_w:.2f} | {r.momentum_block_w:.2f} | {r.trend_block_w:.2f} | "
                f"{r.T3_accuracy:.2%} | {r.T10_ex_T3_accuracy:.2%} | {r.ret_3m:.2%} | {r.ret_6m:.2%} | {r.ret_1y:.2%} | {r.mdd_1y:.2%} |"
            )
        lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    ...
    p = read_sql(
        S3_DB,
        "SELECT ticker, date, close, vol_ratio_20, mom20, breakout60, ma60, ma120, ma60_slope FROM s3_price_features_daily",
        parse_dates=["date"],
    )
    p["ticker"] = p["ticker"].astype(str).str.zfill(6)
    f = read_sql(
        S3_DB,
        "SELECT date, ticker, available_from, growth_score, fund_accel_score FROM s3_fund_features_monthly",
    ...
    variant_meta = pd.DataFrame(specs)
    variant_meta.to_csv(OUTDIR / "s3_single_axis_variant_meta.csv", index=False, encoding="utf-8-sig")
    ...
    for d0 in signal_dates:
        ...
        for spec in specs:
            ...
            chosen["variant_id"] = str(spec["variant_id"])
            chosen["signal_date"] = d0
            chosen["selected"] = 1
            selected_rows.append(chosen[["variant_id", "signal_date", "ticker", "s3_score", "selected"]])
    selected = pd.concat(selected_rows, ignore_index=True)
    selected.to_csv(OUTDIR / "s3_single_axis_selected_history.csv", index=False, encoding="utf-8-sig")
    ...
    for variant_id, vg in selected.groupby("variant_id"):
        ...
        for i in range(1, len(signal_dates)):
            ...
            if held:
                ...
                port_ret = 0.0
            nav *= (1.0 + port_ret)
            series.append({"variant_id": variant_id, "date": d, "nav": nav, "holdings_count": len(held)})
    ...
    nav_all.to_csv(OUTDIR / "s3_single_axis_nav_history.csv", index=False, encoding="utf-8-sig")
    perf_all.to_csv(OUTDIR / "s3_single_axis_performance_summary_long.csv", index=False, encoding="utf-8-sig")

    perf_pivot = perf_all.pivot(index="variant_id", columns="period", values=["total_return", "mdd", "sharpe"])
    perf_pivot.columns = [f"{a}_{b.lower()}" for a, b in perf_pivot.columns]
    perf_pivot = perf_pivot.reset_index()

    bucket_panel = build_bucket_panel()
    accuracy = summarize_accuracy(selected, bucket_panel)

    summary = variant_meta.merge(accuracy, on="variant_id", how="left").merge(perf_pivot, on="variant_id", how="left")
    summary = summary.rename(columns={"total_return_3m": "ret_3m", "total_return_6m": "ret_6m", "total_return_1y": "ret_1y", "mdd_1y": "mdd_1y"})
    summary.to_csv(OUTDIR / "s3_single_axis_sensitivity_summary.csv", index=False, encoding="utf-8-sig")

    ranked_groups = []
    for axis, ag in summary.groupby("axis"):
        sg = ag.copy()
        sg["rank_key"] = (
            sg["T3_accuracy"].fillna(0) * 4.0
            + sg["T10_ex_T3_accuracy"].fillna(0) * 3.0
            + sg["ret_6m"].fillna(0) * 1.5
            + sg["ret_1y"].fillna(0) * 1.5
            + sg["mdd_1y"].fillna(-1.0)
        )
        ranked_groups.append(sg.sort_values("rank_key", ascending=False))
    ranked_df = pd.concat(ranked_groups, ignore_index=True)
    ranked_df.to_csv(OUTDIR / "s3_single_axis_sensitivity_ranked.csv", index=False, encoding="utf-8-sig")

    (OUTDIR / "s3_single_axis_sensitivity_review.md").write_text(render_md(summary), encoding="utf-8")
    print(ranked_df[["axis", "variant_id", "target_value", "T3_accuracy", "T10_ex_T3_accuracy", "ret_3m", "ret_6m", "ret_1y", "mdd_1y"]].to_string(index=False))
