...
def main() -> None:
    ap = argparse.ArgumentParser(description='Smoke validate S5 neutral backtest.')
    ...
    cmd = [
        ...
        str(PROJECT_ROOT / 'src' / 'backtest' / 'run_backtest_s5_neutral_allocation.py'),
    ...
    summary = outdir / f's5_alloc_summary_{stamp}.csv'
    equity = outdir / f's5_alloc_equity_{stamp}.csv'
    weights = outdir / f's5_alloc_weights_{stamp}.csv'
    trades = outdir / f's5_alloc_trades_{stamp}.csv'
    compare = outdir / f's5_vs_task04_neutral_{stamp}.csv'
    ...
    if summary_df.empty or equity_df.empty or weights_df.empty or compare_df.empty:
        raise AssertionError('One or more S5 outputs are empty')
    ...
    if not {'adx_low','vol_contraction','oversold','rebound','uncertainty','broad_rsi'}.issubset(weights_df.columns):
        raise AssertionError('weights diagnostics missing')
    if set(compare_df['model']) != {'TASK04_NEUTRAL','S5_NEUTRAL_V1'}:
