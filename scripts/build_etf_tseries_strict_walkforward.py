...
TRANSITION_PATH = PROJECT_ROOT / r"reports\model_upgrade_research\20260331\ETF_T_SERIES_TRANSITION_RESEARCH\etf_tseries_transition_panel.csv"
OUTDIR = PROJECT_ROOT / r"reports\model_upgrade_research\20260331\ETF_T_SERIES_STRICT_WALKFORWARD"
...
def main() -> None:
    ...
    overall.to_csv(OUTDIR / 'etf_tseries_strict_walkforward_overall.csv', index=False, encoding='utf-8-sig')
    all_window.to_csv(OUTDIR / 'etf_tseries_strict_walkforward_by_window.csv', index=False, encoding='utf-8-sig')
    all_preds.to_csv(OUTDIR / 'etf_tseries_strict_walkforward_top_picks.csv', index=False, encoding='utf-8-sig')

    lines = []
    lines.append('# ETF T-series Strict Walk-Forward Validation')
    lines.append('')
    lines.append(f'- minimum training windows: {MIN_TRAIN_WINDOWS}')
    lines.append('- method: expanding train / one-month forward test / no future leakage')
    lines.append('')
    ...
    (OUTDIR / 'etf_tseries_strict_walkforward.md').write_text('\n'.join(lines), encoding='utf-8')
