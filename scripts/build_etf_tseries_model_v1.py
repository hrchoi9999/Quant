...
ETF_UNIVERSE_CSV = PROJECT_ROOT / r"data\universe\universe_etf_core_20260325.csv"
ETF_REPORT_DIR = PROJECT_ROOT / r"reports\backtest_etf_allocation"
OUTDIR = PROJECT_ROOT / r"reports\model_upgrade_research\20260331\ETF_T_SERIES_V1"
...
def _eval_topn(pred_df: pd.DataFrame, top_n: int) -> tuple[float, float, float, float]:
    rows = []
...
def _train_and_eval(panel: pd.DataFrame, bucket_df: pd.DataFrame) -> tuple[pd.DataFrame, dict[tuple[str, str], Pipeline], pd.DataFrame]:
    ...
    for horizon in HORIZON_STEPS:
        ...
        for target_name, top_n, label_fn in [
            ("ET10", 2, lambda s: s.isin(["ET3", "ET10_ex_ET3"])),
            ("ET3", 1, lambda s: s.eq("ET3")),
        ]:
            df = h_panel.copy()
            ...
            precision, capture, lift, base = _eval_topn(pred_df, top_n)
            summary_rows.append({
                "horizon": horizon,
                "target": target_name,
                "test_auc": auc,
...
def main() -> None:
    ...
    latest_rank.to_csv(OUTDIR / "etf_tseries_latest_full_rank_2026-03-26.csv", index=False, encoding="utf-8-sig")
    latest_rank.head(cut3).to_csv(OUTDIR / "etf_tseries_predicted_top3_2026-03-26.csv", index=False, encoding="utf-8-sig")
    latest_rank.head(cut10).to_csv(OUTDIR / "etf_tseries_predicted_top10_2026-03-26.csv", index=False, encoding="utf-8-sig")
    latest_rank.head(cut30).to_csv(OUTDIR / "etf_tseries_predicted_top30_2026-03-26.csv", index=False, encoding="utf-8-sig")
    latest_rank.head(cut50).to_csv(OUTDIR / "etf_tseries_predicted_top50_2026-03-26.csv", index=False, encoding="utf-8-sig")

    try:
        with pd.ExcelWriter(OUTDIR / "etf_tseries_pack_2026-03-26.xlsx") as writer:
    ...
    lines = [
        ...
        "- universe: `D:/Quant/data/universe/universe_etf_core_20260325.csv` (20 ETFs)",
        ...
        f"- {OUTDIR / 'etf_tseries_latest_full_rank_2026-03-26.csv'}",
    ]
    (OUTDIR / "etf_tseries_model_v1.md").write_text("\n".join(lines), encoding="utf-8")
