...
def main() -> None:
    ap = argparse.ArgumentParser(description='Smoke validate S6 defensive backtest.')
    ...
    cmd = [
        ...
        str(PROJECT_ROOT / 'src' / 'backtest' / 'run_backtest_s6_defensive_allocation.py'),
    ...
    summary = outdir / f's6_alloc_summary_{stamp}.csv'
    equity = outdir / f's6_alloc_equity_{stamp}.csv'
    weights = outdir / f's6_alloc_weights_{stamp}.csv'
    trades = outdir / f's6_alloc_trades_{stamp}.csv'
    compare = outdir / f's6_vs_task04_riskoff_{stamp}.csv'
    ...
    if summary_df.empty or equity_df.empty or weights_df.empty or compare_df.empty:
        raise AssertionError('One or more S6 outputs are empty')
    ...
    if not {'market_vol_20d','crash','usd_strong','gold_strong','bond_long_friendly'}.issubset(weights_df.columns):
        raise AssertionError('weights diagnostics missing')
    if set(compare_df['model']) != {'TASK04_RISK_OFF','S6_DEFENSIVE_V1'}:
