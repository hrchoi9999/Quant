...
WEIGHT_SPECS = [
    {"weighting": "none", "recent_start": None, "recent_weight": 1.0},
    {"weighting": "recent_2022_x2", "recent_start": "2022-01-01", "recent_weight": 2.0},
    {"weighting": "recent_2023_x2", "recent_start": "2023-01-01", "recent_weight": 2.0},
    {"weighting": "recent_2023_x3", "recent_start": "2023-01-01", "recent_weight": 3.0},
]
...
def _sample_weight(train: pd.DataFrame, spec: dict[str, Any]) -> np.ndarray | None:
    if spec["weighting"] == "none":
        return None
    if spec["recent_start"] == "linear":
        dates = pd.to_datetime(train["event_date"], errors="coerce")
        span = max(1, (dates.max() - dates.min()).days)
        age = (dates - dates.min()).dt.days.fillna(0) / span
        return (1.0 + age * (float(spec["recent_weight"]) - 1.0)).to_numpy()
    weights = np.ones(len(train), dtype=float)
    weights[train["event_date"] >= pd.Timestamp(str(spec["recent_start"]))] = float(spec["recent_weight"])
    return weights


def _fit(train: pd.DataFrame, numeric: list[str], categorical: list[str], sample_weight: np.ndarray | None) -> Pipeline:
...
def _evaluate(df: pd.DataFrame, feature_set: str, weight_spec: dict[str, Any], train_end: str, valid_start: str, valid_end: str) -> dict[str, Any]:
    train, valid = _split(df, train_end, valid_start, valid_end)
    numeric, categorical = _feature_columns(train, feature_set)
    weights = _sample_weight(train, weight_spec)
    model = _fit(train, numeric, categorical, weights)
    ...
    return {
        ...
        "weighting": weight_spec["weighting"],
        "recent_start": weight_spec["recent_start"],
        "recent_weight": weight_spec["recent_weight"],
...
def run_ablation(asof: str, train_end: str, valid_start: str) -> dict[str, Any]:
    ...
    for feature_set, frame in [("BASE", base), ("QM_RISK", qm)]:
        for spec in WEIGHT_SPECS:
            rows.append(_evaluate(frame, feature_set, spec, train_end, valid_start, asof))
    results = pd.DataFrame(rows).sort_values(["auc", "top_bottom_positive_spread"], ascending=False)
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    token = asof.replace("-", "")
    csv_path = REPORT_DIR / f"downside_risk_recent_weight_ablation_{token}.csv"
    json_path = REPORT_DIR / f"downside_risk_recent_weight_ablation_{token}.json"
    md_path = REPORT_DIR / f"downside_risk_recent_weight_ablation_{token}.md"
    results.to_csv(csv_path, index=False, encoding="utf-8-sig")
    payload = {
        "source_name": "downside_risk_recent_weight_ablation",
    ...
    md_path.write_text(
        "\n".join(
            [
                f"# Downside Risk Recent Weight Ablation - {asof}",
                "",
                "| feature_set | weighting | auc | top30_pos | bottom30_pos | spread | top30_ret | top30_mdd |",
                "|---|---|---:|---:|---:|---:|---:|---:|",
                *[
                    "| {feature_set} | {weighting} | {auc} | {top30_positive_rate} | {bottom30_positive_rate} | {top_bottom_positive_spread} | {top30_avg_1m_return} | {top30_avg_1m_mdd} |".format(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run recent-weight ablation for AI-DOWNSIDE-RISK-V01.")
