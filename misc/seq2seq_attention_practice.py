# English filename: seq2seq_attention_practice_py.py
# Original path: [능률협회]머신러닝 기반 증권모델링 및 트레이딩 과정 (수강생)/15일차 딥러닝 시계열 분석 실습_ETF실습/1. 금융시계열_LSTM과Atttention/2. Attention/seq2seq_attention-실습.py
# Original filename: seq2seq_attention-실습.py
...
def seq2seq_attention(feature_len=1, after_day=1, input_shape=(20, 1), time_step=20):
    ...
    all_outputs = Lambda(lambda x: K.concatenate(x, axis=1), output_shape=(None, 5 * len(all_outputs)))(all_outputs)
