# English filename: dstp_rnn_cpu_py.py
# Original path: [능률협회]머신러닝 기반 증권모델링 및 트레이딩 과정 (수강생)/15일차 딥러닝 시계열 분석 실습_ETF실습/1. 금융시계열_LSTM과Atttention/2. Attention/DSTP_RNN_cpu(오래걸림).py
# Original filename: DSTP_RNN_cpu(오래걸림).py

# ---- Cell ----
!wget -nc http://youngminhome.iptime.org:5555/shared/paper-implementation-dstp-rnn-for-stock-prediction-based-on-da-rnn-master/2324.TW.csv
!wget -nc http://youngminhome.iptime.org:5555/shared/paper-implementation-dstp-rnn-for-stock-prediction-based-on-da-rnn-master/ops.py
...
class Encoder(nn.Module):
    ...
    def forward(self, X ,y_prev):
        ...
        for t in range(self.T - 1):
            ...
            # softmax로 가중치 얻는다. (get weights by softmax)
            alpha = F.softmax(x.view(-1, self.input_size))# 233x363

            # LSTM을 위한 새로운 입력(get new input for LSTM)
            x_tilde = torch.mul(alpha, X[:, t, :]) #233x363
            # print(x_tilde.size())

            # 인코더 LSTM (encoder LSTM)
            ...
            # 2단계 어켄션 (Phase two attention from DSTP-RNN Paper)
...
class DSTP_rnn(nn.Module):
    ...
    def train_forward(self, X, y_prev, y_gt):
        ...
        input_weighted, input_encoded = self.Encoder(
            ...
            Variable(torch.from_numpy(y_prev).type(torch.FloatTensor))) #cuda
    ...
    def test(self, on_train=False):
        ...
        while i < len(y_pred_price):
            ...
            _, input_encoded = self.Encoder(Variable(torch.from_numpy(X).type(torch.FloatTensor)),Variable(y_history)) #cuda
...
model = DSTP_rnn(X, y, 10 , 128, 128, 128, 0.001, 1000) #7000
model.train()
