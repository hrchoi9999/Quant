# English filename: lstm_cnn_multi_py.py
# Original path: [능률협회]머신러닝 기반 증권모델링 및 트레이딩 과정 (수강생)/15일차 딥러닝 시계열 분석 실습_ETF실습/1. 금융시계열_LSTM과Atttention/1. LSTM/LSTM_CNN_Multi.py
# Original filename: LSTM_CNN_Multi.py

# ---- Cell ----
!wget -nc http://youngminhome.iptime.org:5555/shared/lstm-cnn-stock-master/Data.zip&& unzip -n Data.zip
...
Lstock = stock.drop(['close','date'],axis=1)
...
plt.show()
stock_final = stock.drop(['date','open','volume','macd','bb_bbm','bb_bbh','bb_bbl','ichi_a','ichi_conv'],axis=1)
