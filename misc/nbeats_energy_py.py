# English filename: nbeats_energy_py.py
# Original path: [능률협회]머신러닝 기반 증권모델링 및 트레이딩 과정 (수강생)/15일차 딥러닝 시계열 분석 실습_ETF실습/2. 최신딥러닝시계열모델/NBEATS_energy_clear.py
# Original filename: NBEATS_energy_clear.py
...
ts_tpred = model.predict(   n=len(ts_ttest) + 1,
                            past_covariates=cov_t,
                            num_samples=N_SAMPLES,
                            verbose=True,
                            n_jobs=N_JOBS)
